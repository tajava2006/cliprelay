...
```
Nostr 릴레이 → Nostr 서비스 (백그라운드) → 로컬 저장소 → UI
OS 클립보드 → 클립보드 모니터 → Nostr 서비스 → 릴레이
```

- **UI는 Nostr 릴레이를 절대 직접 참조하지 않는다.** 로컬 저장소만 구독한다.
- **Nostr 서비스가 백그라운드에서 릴레이를 구독하고 발행한다.**
- **클립보드 모니터와 Nostr 서비스의 생명주기는 UI와 독립적이다.**
...
- `../web-parser` — 동일한 Nostr 패턴(NIP-46, NIP-65, NIP-44, NIP-78)으로 구현된 별도 앱. 릴레이 디스커버리, 저장소 구독 패턴, QR코드 NIP-46 로그인, 이벤트 서명 위임 패턴을 참조할 것.
