...
/**
 * NIP-46 부트스트랩 릴레이 — 로그인 handshake + signer 세션 랑데부.
 * 클립보드 동기화 릴레이(RELAY_LIST_KIND)와 무관.
 *
 * 로그인 URI에 실리므로 벙커(Amber)도 이 세션용으로 구독하는 릴레이들이다.
 * skipSwitchRelays로 세션 릴레이를 이 목록으로 유지하고(벙커 선호 목록으로
 * 갈아치우지 않음), 기존 세션 복원 시에도 저장된 릴레이에 이 목록을 합쳐
