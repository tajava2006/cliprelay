 ...
 * macOS App Nap의 타이머 코얼레싱이 ~10초 간격이라 10초 임계값은 드리프트 ~9초로
 * 아슬하게 빠져나간다 — 그 동안 소켓은 조용히 죽는다. 5초로 잡아야 걸린다.
 * (정상 동작 중 1초 타이머가 5초 이상 밀리는 일은 없다; 오탐이어도 비용은
 * 재구독 한 번 + 10초 쿨다운이라 감수할 만하다.)
 */
