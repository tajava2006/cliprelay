...
**완료:** SharedPreferences 사용 코드 제거. 이후 Intent extras로 암호문을 직접 ClipboardActionActivity에 전달하는 방식으로 통합됨(H-6 참조).
...
**위치:** `app/src-tauri/plugins/tauri-plugin-foreground-service/android/.../ClipboardSyncService.kt:83-84`
...
**권장:** ClipboardActionActivity에 `android:exported="false"` 확인. Intent 대신 메모리 내 전달(서비스 싱글턴에서 in-memory 핸드오프) 고려.
...
### [M-8] SyncBridge @Volatile 경합 — MEDIUM ✅ 무효

`SyncBridge`는 `consumePendingCopy` 커맨드가 유일한 사용처였고 두 필드 모두 실제로 write되는 경로가 없어 dead code였다. 파일 전체를 삭제하여 이 이슈는 소멸.
...
`nextNativeNotificationId`가 `Int.MAX_VALUE`를 넘으면 오버플로우. 현실적으로 21억 회 이상의 알림 후에나 발생하지만 방어 코드가 없다.

**권장:** 최소한 overflow 방어 또는 순환 범위(200-10000) 사용.
