...
### [C-3] CSP(Content Security Policy) 비활성화 — CRITICAL
...
### [H-4] Android SharedPreferences에 히스토리 평문 저장 — HIGH
...
| ID | 제목 |
|----|------|
| C-2 | 수신 이벤트 서명 미검증 → verifyEvent() 추가 |
| C-3 | CSP 비활성화 → strict CSP 설정 |

### P1 — 다음 릴리스 전
| ID | 제목 |
|----|------|
| H-0 | 클라이언트 개인키 평문 저장 → OS 키체인 |
| H-1 | HTTP 접근 범위 제한 |
| H-2 | hexToBytes 검증 + 중복 제거 |
| H-3 | 복호화 페이로드 런타임 타입 검증 |
| H-4 | SharedPreferences 암호화 |
| H-5 | 프로덕션 debug 로그 제거 |
| H-6 | Intent extras 민감 데이터 제거 |
| H-7 | decryptPayload 제네릭 안전성 |
| H-8 | 발행 경합 방어 |

### P2 — 개선
| ID | 제목 |
|----|------|
| M-1 ~ M-10 | 메모리 누수, 폴링 최적화, debounce, 타임아웃, 코드 품질 |

### P3 — 참고
| ID | 제목 |
|----|------|
| L-1 ~ L-4 | UX 피드백, 오버플로우 방어, 알림 권한 |
